CREATE OR REPLACE FUNCTION functions.inventory_adjunct_mapping_update(
    -- IN VARIABLES
...
BEGIN
    ...
    IF i_inventory_hop_id IS NULL THEN
        ...
        USING MESSAGE = 'EMPTY_INVENTORY_ADJUNCT_ID',
            ...
            HINT = 'inventory_adjunct_mapping_update';
    END IF;
    ...
    FOREACH l_map_uuid IN ARRAY i_maps LOOP
        ...
        UPDATE
            data.inventory_adjunct_mapping
        SET
            inventory_adjunct_id = i_inventory_adjunct_id,
            updated = NOW()
        WHERE
            id = l_map_uuid;
...
ALTER FUNCTION functions.inventory_adjunct_mapping_update OWNER TO shbf_writer;
